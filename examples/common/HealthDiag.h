/**
 * @file HealthDiag.h
 * @brief Verbose health diagnostic helpers for INA228 examples.
 *
 ...
 */
inline const char* stateToString(INA228::DriverState state) {
  switch (state) {
    case INA228::DriverState::UNINIT:   return "UNINIT";
    case INA228::DriverState::READY:    return "READY";
    case INA228::DriverState::DEGRADED: return "DEGRADED";
    case INA228::DriverState::OFFLINE:  return "OFFLINE";
 ...
 */
inline const char* stateColor(INA228::DriverState state) {
  switch (state) {
    case INA228::DriverState::READY:    return LOG_COLOR_GREEN;
    case INA228::DriverState::DEGRADED: return LOG_COLOR_YELLOW;
    case INA228::DriverState::OFFLINE:  return LOG_COLOR_RED;
    case INA228::DriverState::UNINIT:   return LOG_COLOR_GRAY;
 ...
 */
inline void printHealthOneLine(INA228::INA228& driver) {
  INA228::DriverState st = driver.state();
 ...
 */
inline void printHealthVerbose(INA228::INA228& driver) {
  INA228::DriverState st = driver.state();
  INA228::Status lastErr = driver.lastError();
  uint32_t now = millis();
...
struct HealthSnapshot {
  INA228::DriverState state;
  uint8_t consecutiveFailures;
  uint32_t totalSuccess;
  uint32_t totalFailures;
  uint32_t timestamp;

  void capture(INA228::INA228& driver) {
    state = driver.state();
...
public:
  ...
  void begin(uint32_t intervalMs = 1000) {
    ...
    _lastLogMs = 0;
    _lastState = INA228::DriverState::UNINIT;
    _lastConsecFail = 0;
  }
   ...
   */
  void tick(INA228::INA228& driver, bool forceLog = false) {
    uint32_t now = millis();
    INA228::DriverState currentState = driver.state();
...
private:
  ...
  INA228::DriverState _lastState = INA228::DriverState::UNINIT;
