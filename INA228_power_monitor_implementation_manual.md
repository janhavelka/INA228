...
|---|---------------|----------|-----------------|-------|------|
| 1 | INA228 85-V, 20-Bit, Ultra-Precise Power/Energy/Charge Monitor With I2C Interface | `datasheet_INA228.pdf` | SLYS021A (Jan 2021, Rev May 2022) | 48 | Primary datasheet |
...
| Field | Value | Source |
|-------|-------|--------|
| Device name | INA228 | datasheet_INA228.pdf, p1 |
| Full title | 85-V, 20-Bit, Ultra-Precise Power/Energy/Charge Monitor With I2C Interface | datasheet_INA228.pdf, p1 |
| Automotive variant | INA228-Q1 | datasheet_INA228.pdf, p1 (title mentions INA228; Q1 implied by revision note referencing "commercial data sheet") |
...
| Package | VSSOP-10 (DGS) | datasheet_INA228.pdf, p1 |
| Body size | 3.00 mm × 3.00 mm | datasheet_INA228.pdf, p1 |
| Pin count | 10 | datasheet_INA228.pdf, p3 |
| Manufacturer ID register value | 0x5449 ("TI" in ASCII) | datasheet_INA228.pdf, p29 |
| Device ID register value | 0x2281 (DIEID = 0x228, REV_ID = 0x1) | datasheet_INA228.pdf, p29 |
| TI literature number | SLYS021A | datasheet_INA228.pdf, p1 |
...
|-----------|-------|--------|
| Interface type | I2C (also SMBus compatible) | datasheet_INA228.pdf, p18 |
| Bus role | Secondary (slave) only | datasheet_INA228.pdf, p18 |
| Fast mode clock range | 1 kHz to 400 kHz | datasheet_INA228.pdf, p7, p19 |
| High-speed mode clock range | 1 kHz to 2.94 MHz | datasheet_INA228.pdf, p7, p19 |
| Address pins | A0 (pin 2), A1 (pin 1) | datasheet_INA228.pdf, p3 |
| Number of selectable addresses | 16 | datasheet_INA228.pdf, p19 |
| Address pin connection options | GND, VS, SDA, SCL (4 states per pin) | datasheet_INA228.pdf, p19 |
| SDA type | Open-drain bidirectional | datasheet_INA228.pdf, p3 |
| SCL type | Digital input | datasheet_INA228.pdf, p3 |
| Packet Error Checking (PEC) | Not supported | datasheet_INA228.pdf, p20 |
| Clock stretching | Not supported | datasheet_INA228.pdf, p20 |
| SMBus Alert Response | Supported (address 0001 100 with R/W high) | datasheet_INA228.pdf, p21 |
| Byte order | MSB first | datasheet_INA228.pdf, p19 |
| Integrated spike suppression | Yes (on SDA and SCL) | datasheet_INA228.pdf, p18 |
...
(datasheet_INA228.pdf, p19, Table 7-2)

**Note:** The device samples A0 and A1 on every bus communication. When SDA is connected to A0 or A1, an additional hold time of 100 ns is needed on the MSB of the I2C address to ensure correct device addressing. (datasheet_INA228.pdf, p19)
...
(datasheet_INA228.pdf, p20–21)

### I2C Timing — Fast Mode (datasheet_INA228.pdf, p7)
...
### I2C Timing — High-Speed Mode (datasheet_INA228.pdf, p7)
...
### Absolute Maximum Ratings (datasheet_INA228.pdf, p3)
...
### Addendum: Absolute Maximum Ratings — Missing Entries (datasheet_INA228.pdf, p3)
...
**Note:** Stresses beyond those listed under Absolute Maximum Ratings may cause permanent damage to the device. These are stress ratings only, which do not imply functional operation of the device at these or any other conditions beyond those indicated under Recommended Operating Conditions. Exposure to absolute-maximum-rated conditions for extended periods may affect device reliability. (datasheet_INA228.pdf, p3)

### Addendum: ESD Ratings (datasheet_INA228.pdf, p4)
...
- JEDEC document JEP155 states that 500-V HBM allows safe manufacturing with a standard ESD control process. (datasheet_INA228.pdf, p4)
- JEDEC document JEP157 states that 250-V CDM allows safe manufacturing with a standard ESD control process. (datasheet_INA228.pdf, p4)

### Addendum: Thermal Information (datasheet_INA228.pdf, p4)
...
### Recommended Operating Conditions (datasheet_INA228.pdf, p4)
...
### Key Electrical Characteristics (datasheet_INA228.pdf, p5–6)
...
### Digital I/O Levels (datasheet_INA228.pdf, p6)
...
### ADC LSB Step Sizes (datasheet_INA228.pdf, p5)
...
### Power Supply (datasheet_INA228.pdf, p5–6, p39)
...
**Power-on reset (POR):** POR is asserted when VS drops below 1.26 V (typical), at which point all registers are reset to their default values. (datasheet_INA228.pdf, p18)

**Software reset:** Setting the RST bit (bit 15) in CONFIG register (0x00) to 1 generates a system reset equivalent to POR. This bit self-clears. (datasheet_INA228.pdf, p22)

**Accumulator reset:** Setting the RSTACC bit (bit 14) in CONFIG register (0x00) to 1 clears the ENERGY and CHARGE registers to 0. (datasheet_INA228.pdf, p22)

**Supply sequencing:** No special power-supply sequencing required. The common-mode input range and device supply voltage are independent; bus voltage can be present with supply off and vice-versa without damage. (datasheet_INA228.pdf, p12)

**Supply bypass:** Place 0.1 µF bypass capacitor as close as possible to VS and GND pins. Noisy or high-impedance supplies may need additional decoupling. (datasheet_INA228.pdf, p39)

**Oscillator stabilization:** On power up, the internal oscillator and ADC take ~300 µs to reach <1% error stability. Once stabilized, ADC data output is accurate to spec. (datasheet_INA228.pdf, p16)

**GND caution:** Avoid applications where the GND pin is disconnected while device is actively powered. (datasheet_INA228.pdf, p39)

---

## 7. Pin Behavior Relevant to Firmware

### Pin Map (datasheet_INA228.pdf, p3, Table 5-1)
...
### ALERT Pin Behavior (datasheet_INA228.pdf, p16–18)
...
- **Fast alert response:** 75 µs (datasheet_INA228.pdf, p1)
- **Alert response timing:** For fault conditions near threshold, response is 0.5 to 1.5 conversion cycles. For faults significantly above threshold, response can be as fast as ¼ conversion time. Applications where alert timing is critical should assume 1.5 × ADC conversion time. (datasheet_INA228.pdf, p37)
...
- **SMBus Alert Response:** Device responds to Alert Response address (0001100 with R/W=1) by acknowledging and sending its own address on the bus. (datasheet_INA228.pdf, p21)

### VBUS Pin (datasheet_INA228.pdf, p5)

- Input impedance (active mode): 0.8–1.2 MΩ (typ 1 MΩ)
- Leakage current (shutdown, VBUS=85V): 10 nA max
- Can be left floating if unused (layout example note, datasheet_INA228.pdf, p39)

### IN+/IN− Pins

- Input bias current: 0.1 nA typ, 2.5 nA max at VCM=85V (datasheet_INA228.pdf, p5)
- Differential input impedance (active): 92 kΩ (datasheet_INA228.pdf, p5)
- Tolerates full 0–85V common-mode even with VS off (datasheet_INA228.pdf, p39)
...
(datasheet_INA228.pdf, p21, Table 7-3)

**Note:** All register locations not listed should be considered reserved; contents should not be modified. (datasheet_INA228.pdf, p21)

**Note on register widths:** 16-bit registers require 2 data bytes on I2C. 24-bit registers require 3 data bytes. 40-bit registers (ENERGY, CHARGE) require 5 data bytes. All transferred MSB first. (datasheet_INA228.pdf, p19–20)
...
(datasheet_INA228.pdf, p22, Table 7-5)
...
(datasheet_INA228.pdf, p23–24, Table 7-6)
...
(datasheet_INA228.pdf, p24, Table 7-7)
...
(datasheet_INA228.pdf, p30–31, Equations 2 and 3)
...
(datasheet_INA228.pdf, p24, Table 7-8)

**Temperature compensation formula (Equation 1, datasheet_INA228.pdf, p16):**
```
...
(datasheet_INA228.pdf, p25, Table 7-9)
...
(datasheet_INA228.pdf, p25, Table 7-10)
...
(datasheet_INA228.pdf, p25, Table 7-11)
...
(datasheet_INA228.pdf, p25, Table 7-12)

**Current calculation (Equation 4, datasheet_INA228.pdf, p31):**
```
...
(datasheet_INA228.pdf, p25–26, Table 7-13)

**Power calculation (Equation 5, datasheet_INA228.pdf, p31):**
```
...
(datasheet_INA228.pdf, p26, Table 7-14)

**Energy calculation (Equation 6, datasheet_INA228.pdf, p31):**
```
Energy [J] = 16 × 3.2 × CURRENT_LSB × ENERGY
```

**Overflow:** The ENERGYOF bit (bit 11) in DIAG_ALRT indicates overflow. Clears when ENERGY register is read. Register can be reset via RSTACC bit. (datasheet_INA228.pdf, p27)
...
(datasheet_INA228.pdf, p26, Table 7-15)

**Charge calculation (Equation 7, datasheet_INA228.pdf, p32):**
```
Charge [C] = CURRENT_LSB × CHARGE
```

**Overflow:** The CHARGEOF bit (bit 10) in DIAG_ALRT indicates overflow. Clears when CHARGE register is read. Register can be reset via RSTACC bit. (datasheet_INA228.pdf, p27)
...
(datasheet_INA228.pdf, p26–28, Table 7-16)
...
(datasheet_INA228.pdf, p28, Table 7-17)

**Caveat:** If negative values are entered, a shunt voltage measurement of 0 V will trip this alarm. When using negative values for shunt thresholds, the overvoltage threshold must be the larger (less negative) of the two. (datasheet_INA228.pdf, p28)
...
(datasheet_INA228.pdf, p28, Table 7-18)
...
(datasheet_INA228.pdf, p28, Table 7-19)
...
(datasheet_INA228.pdf, p28–29, Table 7-20)
...
(datasheet_INA228.pdf, p29, Table 7-21)
...
(datasheet_INA228.pdf, p29, Table 7-22)

**Note on power limit LSB:** The power limit register is 16 bits while the POWER register is 24 bits, so the effective LSB for the limit register is 256× the power LSB. (datasheet_INA228.pdf, p37)
...
(datasheet_INA228.pdf, p29, Table 7-23)
...
(datasheet_INA228.pdf, p29, Table 7-24)
...
(datasheet_INA228.pdf, p20, Figure 7-7)
...
(datasheet_INA228.pdf, p19–20, Figures 7-8 and 7-9)
...
The device retains the register pointer value until changed by the next write operation. Repeated reads from the same register do not require resending the register pointer. (datasheet_INA228.pdf, p20)

### Transaction Notes
- The device does **not** support Packet Error Checking (PEC). (datasheet_INA228.pdf, p20)
- The device does **not** perform clock stretching. (datasheet_INA228.pdf, p20)
- All data bytes are transmitted MSB first following SMBus 3.0 transfer protocol. (datasheet_INA228.pdf, p19)
...
1. **Wait for POR completion:** Wait at least 300 µs after VS is applied for oscillator and ADC stabilization. (datasheet_INA228.pdf, p6, p16)
...
3. **Check memory status:** Read DIAG_ALRT (0x0B), verify MEMSTAT (bit 0) = 1 for normal operation. (datasheet_INA228.pdf, p28)
...
5. **Configure ADC_CONFIG register (0x01):**
   ...
   - **Note:** Writing ADC_CONFIG clears the CNVRF flag and restarts conversions (except shutdown mode). (datasheet_INA228.pdf, p14)

6. **Program SHUNT_CAL register (0x02):**
   - Calculate: `SHUNT_CAL = 13107.2 × 10^6 × CURRENT_LSB × RSHUNT`
   - For ADCRANGE = 1, multiply by 4
   - **Must be programmed at every VS power cycle.** Failure to set results in zero for current/power/energy/charge. (datasheet_INA228.pdf, p36)
...
8. **Set alert thresholds (if using alerts):**
   ...
   - **All threshold registers reset to defaults on VS power cycle and must be reprogrammed.** (datasheet_INA228.pdf, p37)
...
Same sequence as power-on, but POR wait may be shorter (device is already powered). All registers are reset to defaults. (datasheet_INA228.pdf, p18, p22)
...
- **ENERGY and CHARGE registers are invalid in triggered mode** because the device does not track elapsed time. (datasheet_INA228.pdf, p14)
- Writing the MODE bits will interrupt and restart any triggered or continuous conversion in progress. (datasheet_INA228.pdf, p14)

#### 3. Shutdown Mode (MODE = 0x0 or 0x8)
- Quiescent current drops to < 5 µA (max). (datasheet_INA228.pdf, p5)
- Current into device inputs is turned off. (datasheet_INA228.pdf, p18)
- Registers can still be read and written via I2C. (datasheet_INA228.pdf, p18)
- Device remains in shutdown until triggered or continuous conversion command is received.
- Active I2C clock/data activity increases shutdown current as a function of bus frequency. (datasheet_INA228.pdf, p18, Figure 6-23)
- Wake-up from shutdown: ~60 µs. (datasheet_INA228.pdf, p6)

### Conversion Sequencing

Enabled measurement inputs are converted **sequentially** through the internal multiplexer: shunt voltage → bus voltage → temperature (order determined by which are enabled). (datasheet_INA228.pdf, p12, p14)
...
- Default: 0 (no delay).
- Useful for synchronizing multiple devices. Note: synchronization will drift over time due to oscillator mismatch. (datasheet_INA228.pdf, p14)

### Data Validity
- Without averaging: registers are updated immediately after each conversion. (datasheet_INA228.pdf, p13)
- With averaging: intermediate values are stored in accumulator; output registers are only updated after all averages are collected. Values in output registers remain valid until replaced by next completed conversion. (datasheet_INA228.pdf, p14)
- Reading data output registers does not affect a conversion in progress. (datasheet_INA228.pdf, p14)
...
(datasheet_INA228.pdf, p12, Figure 7-1)

### Calculation Engine (datasheet_INA228.pdf, p13)
...
- If SHUNT_CAL = 0, power, energy, and charge all report zero. (datasheet_INA228.pdf, p13)
- All calculations are performed in the background and do not add to overall conversion time. (datasheet_INA228.pdf, p13)
...
- Energy and charge are accumulated for **each conversion cycle** (not per average). (datasheet_INA228.pdf, p13)
- The INA228 averaging function is **not applied** to energy and charge. (datasheet_INA228.pdf, p13)
- On overflow, ENERGY and CHARGE registers roll over to zero. (datasheet_INA228.pdf, p32)
- ENERGYOF / CHARGEOF bits in DIAG_ALRT indicate overflow; they clear when the respective register is read. (datasheet_INA228.pdf, p27)
- RSTACC bit in CONFIG register resets both ENERGY and CHARGE to 0. (datasheet_INA228.pdf, p22)
- **Energy and charge are invalid in triggered mode** because elapsed time is not tracked. Use continuous mode for accumulation. (datasheet_INA228.pdf, p14)
...
- Transients at or near the 1 MHz sampling rate harmonics can cause problems; use external input filtering if needed. (datasheet_INA228.pdf, p14–15, p34)

### Input Filtering Recommendations (datasheet_INA228.pdf, p34–35)
...
### Two's Complement Conversion Example (datasheet_INA228.pdf, p37)
...
### Addendum: Explicit LSB Relationships (datasheet_INA228.pdf, p31–32, p37)
...
### Addendum: Maximum Shunt Resistor Formula (datasheet_INA228.pdf, p36, Equation 8)
...
The closest standard resistor value **smaller** than the calculated maximum should be selected. Also ensure RSHUNT can handle the power dissipated at maximum load current. (datasheet_INA228.pdf, p36)

### Addendum: Temperature Sensor Internal Range (datasheet_INA228.pdf, p30)
...
### Addendum: ADC Noise Performance / ENOB Table (datasheet_INA228.pdf, p32–34, Table 8-2)
...
(datasheet_INA228.pdf, p17, Table 7-1)
...
- The CNVRF status bit can be read regardless of CNVR setting. (datasheet_INA228.pdf, p14, p17, p28)

### Multi-Alert Behavior

When CNVR=1 (conversion ready on ALERT), the ALERT pin serves as a multipurpose output. It pulses for conversion-complete events and also asserts for fault conditions. Fault events and conversion-complete events can overlap on the same pin. (datasheet_INA228.pdf, p17–18, Figure 7-6)

### Alert Timing

- **Fast alert response:** 75 µs (from feature list). (datasheet_INA228.pdf, p1)
...
- **Critical timing applications should assume 1.5 × conversion time.** (datasheet_INA228.pdf, p37)
- Variation exists because external fault is not synchronized to internal ADC conversion start. (datasheet_INA228.pdf, p37)

### Addendum: Alert Response Time Measured Values (datasheet_INA228.pdf, p37–38, Figures 8-3 and 8-4)
...
The ADC is constantly sampling, so response time for fault events starting from zero will be slower than fault events starting from values near the set fault threshold. (datasheet_INA228.pdf, p37)
...
**MEMSTAT bit (DIAG_ALRT bit 0):** Monitors health of device NV trim memory via checksum. Should always read 1 during normal operation. A value of 0 indicates a checksum error in the trim memory space. (datasheet_INA228.pdf, p28)
...
1. **SHUNT_CAL = 0 disables all current-based outputs.** If SHUNT_CAL is zero, current, power, energy, and charge all report zero. Must be programmed at every power cycle. (datasheet_INA228.pdf, p13, p36)

2. **Temperature compensation warning.** With TEMPCOMP enabled, if shunt voltage exceeds ~70% of full range AND shunt tempco > 2000 ppm/°C AND temperature > 100°C, reported current will be lower than actual because effective resistance calculated keeps increasing while shunt voltage is clamped at ADC range. (datasheet_INA228.pdf, p16)

3. **Energy and charge invalid in triggered mode.** The device does not track elapsed time in triggered mode. Use continuous mode for accumulation. (datasheet_INA228.pdf, p14)

4. **Writing MODE bits restarts conversions.** Writing the MODE bits interrupts and restarts any triggered or continuous conversion in progress. (datasheet_INA228.pdf, p14)

5. **Conversion time subject to oscillator accuracy.** The ADC conversion times are subject to oscillator accuracy and drift (±0.5% at 25°C, ±1% over temperature). (datasheet_INA228.pdf, p5–6)

6. **SHUNT_CAL bit 15 is reserved.** Only bits 14:0 are writable. (datasheet_INA228.pdf, p24)

7. **VBUS register is two's complement but always positive.** The bus voltage measurement format uses two's complement but the value is always positive (0 V to 85 V). (datasheet_INA228.pdf, p25)

8. **Synchronization drift.** Even with precise internal time base, synchronization between multiple devices will be lost over time due to internal/external time base mismatch. (datasheet_INA228.pdf, p14)

9. **SDA connected to A0/A1 requires extra hold time.** When SDA is connected to A0 or A1 for address selection, add 100 ns hold time on the MSB of the I2C address. (datasheet_INA228.pdf, p19)

10. **Threshold registers reset on power cycle.** All alert limit register values reset to defaults on VS power cycle and must be reprogrammed. (datasheet_INA228.pdf, p37)

11. **Negative SOVL values trigger on 0 V.** If negative values are used in SOVL, a 0 V shunt measurement will trip the alarm. The overvoltage threshold must be less negative than the undervoltage threshold. (datasheet_INA228.pdf, p28)

12. **MATHOF must be manually cleared.** Unlike other flags, MATHOF is not auto-cleared by reading DIAG_ALRT. Clear by triggering another conversion or using RSTACC. (datasheet_INA228.pdf, p27)

13. **ENERGYOF and CHARGEOF clear on register read.** Reading the ENERGY register clears ENERGYOF; reading CHARGE clears CHARGEOF. (datasheet_INA228.pdf, p27)

14. **SLOWALERT delays alert.** When enabled, alert comparison uses the averaged value, which delays the alert by the full averaging period. (datasheet_INA228.pdf, p17)

15. **Input filter series resistors must be ≤100 Ω.** Values greater than 100 Ω degrade gain error and increase non-linearity. (datasheet_INA228.pdf, p35)

16. **No PEC, no clock stretching.** The device does not support I2C Packet Error Checking or clock stretching. (datasheet_INA228.pdf, p20)

17. **Active I2C bus increases shutdown current.** Clock and data activity during shutdown increase current consumption proportional to bus frequency. (datasheet_INA228.pdf, p18)

18. **Shunt offset spec requires TCT > 280 µs.** The ±1 µV max shunt offset voltage specification is valid only when conversion time > 280 µs. (datasheet_INA228.pdf, p5)

### Addendum: Layout Guidelines — Kelvin Connection (datasheet_INA228.pdf, p39)

Connect the input pins (IN+ and IN−) to the sensing resistor using a **Kelvin connection** (4-wire connection). This connection technique ensures that only the current-sensing resistor impedance is sensed between the input pins. Poor routing of the current-sensing resistor commonly results in additional resistance present between the input pins. Given the very low ohmic value of the current-sensing resistor, any additional high-current carrying impedance causes significant measurement errors. (datasheet_INA228.pdf, p39)

### Addendum: Overload and dV/dt Input Protection (datasheet_INA228.pdf, p34–35)
...
- In systems without large energy storage/electrolytic capacitors, excessive dV/dt can activate the ESD protection. Adding **10 Ω series resistors** on each input protects against dV/dt failure up to the 40-V maximum differential rating. (datasheet_INA228.pdf, p34–35)

### Addendum: Typical Application Worked Design Example (datasheet_INA228.pdf, p35–37, Tables 8-3/8-4)
...
**Worked calculations:**
- RSHUNT(max) = 163.84 mV / 10 A = 16.38 mΩ → select 16.2 mΩ standard value (datasheet_INA228.pdf, p36, Eq8)
- CURRENT_LSB = 10 A / 2^19 = 19.0735 µA (datasheet_INA228.pdf, p36)
- SHUNT_CAL = 13107.2 × 10^6 × 19.0735 × 10^-6 × 16.2 × 10^-3 = 4050d = 0x0FD2 (datasheet_INA228.pdf, p36)
- SOVL register for 10-A overcurrent: (10 A × 16.2 mΩ) / (312.5 nV × 16) = 162 mV / 5 µV = 32400d = 0x7E90 (datasheet_INA228.pdf, p36)
- BOVL register for 52-V bus OV: 52 V / (195.3125 µV × 16) = 52 V / 3.125 mV = 16640d = 0x4100 (datasheet_INA228.pdf, p37)
...
(datasheet_INA228.pdf, p37)

### Addendum: Orderable Part Information (datasheet_INA228.pdf, p41)
...
Automotive qualified version: **INA228-Q1** (Q100 qualified for high-reliability automotive applications targeting zero defects). (datasheet_INA228.pdf, p41–42)

---

## 17. Recommended Polling and Control Strategy Hints from the Docs

1. **Use CNVRF to coordinate reads.** The Conversion Ready flag (CNVRF, DIAG_ALRT bit 1) indicates when all conversions and averaging are complete. This is particularly useful for triggered mode. (datasheet_INA228.pdf, p14)

2. **ALERT pin for interrupt-driven operation.** Configure CNVR=1 in DIAG_ALRT to assert ALERT on conversion complete, enabling interrupt-driven reads instead of polling. (datasheet_INA228.pdf, p17)

3. **Reading data registers at any time is safe.** The device can be read at any time; data from the last completed conversion remains available. Reading does not affect conversions in progress. (datasheet_INA228.pdf, p14)

4. **For energy/charge applications, use continuous mode.** Triggered mode does not track time; only continuous mode provides valid energy and charge accumulation. (datasheet_INA228.pdf, p14)

5. **RSTACC to reset accumulators.** Use CONFIG bit 14 (RSTACC) to clear ENERGY and CHARGE registers at known intervals for periodic energy accounting. (datasheet_INA228.pdf, p22)

6. **Use SLOWALERT for noisy environments.** When measurements are noisy, enable SLOWALERT so that alert comparison is performed on the averaged result, preventing false alerts from noise spikes. (datasheet_INA228.pdf, p17)

7. **Use latch mode (ALATCH=1) to prevent missing transient faults.** In transparent mode, the alert auto-clears when the fault condition clears, which could be missed between polls. Latch mode holds the alert until the DIAG_ALRT register is explicitly read. (datasheet_INA228.pdf, p17)

8. **Monitor MEMSTAT at startup.** Verify MEMSTAT=1 after power-up to ensure factory trim data integrity. (datasheet_INA228.pdf, p28)

9. **Monitor MATHOF.** After reading current or power, check MATHOF to verify no arithmetic overflow invalidated the data. (datasheet_INA228.pdf, p27)

10. **Synchronization with CONVDLY.** Use the conversion delay feature to synchronize multiple INA228 devices, but be aware of drift over time. (datasheet_INA228.pdf, p14)

11. **Round CURRENT_LSB for simpler conversion.** While the smallest CURRENT_LSB yields highest resolution, it is common to select a higher round number (up to 8× larger) to simplify software conversion. (datasheet_INA228.pdf, p30)
...
|-------------|----------|----------|
| datasheet_INA228.pdf | INA228 85-V, 20-Bit, Ultra-Precise Power/Energy/Charge Monitor With I2C Interface | SLYS021A (Rev A, May 2022) |
