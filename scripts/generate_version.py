...
Default behavior:
- when run by PlatformIO as an extra script: sync release metadata if needed and
  inject build metadata defines into the compile environment
- when run standalone: sync release metadata if needed

Standalone commands:
  sync
      Synchronize release metadata only if library.json changed.
  check
      Exit with code 1 when synchronized metadata is out of date.
  bump patch|minor|major
      Update library.json, then synchronize release metadata.
  set X.Y.Z
      Set an explicit semantic version, then synchronize release metadata.
"""
